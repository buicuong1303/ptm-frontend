...
import { createLabel } from '../../Label.asyncAction';
...
function CreateLabel() {
  ...
  const handleConfirmDialog = async () => {
    try {
      const result = await dispatch(createLabel(formValue));
      const data = unwrapResult(result);

      showSnackbar('Create label success', 'success');
      ...
      showSnackbar('Create label failed', 'error');
    }
  ...
  return (
    <Page className={classes.root} title="Create label">
      <Header childTitle="Create Label" urlChild="/labels/create" />
      ...
      <DialogConfirm
        ...
        message={'Create new label in system'}
        open={dialogConfirm}
      />
    </Page>
  );
