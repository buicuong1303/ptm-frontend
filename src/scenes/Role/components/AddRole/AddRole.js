...
import { addRole } from 'scenes/Role/Role.asyncActions';
...
const AddRole = (props) => {
  const { change, open, permission, ...rest } = props;
  ...
  const handleConfirmDialog = () => {
    dispatch(addRole(formikManagement.values));
    handleCloseAddRole();
  ...
  useEffect(() => {
    const setValuePermission = async () => {
      if (formikManagement) {
        await formikManagement.setFieldValue('permissionAdd', checked);
      }
    };
    setValuePermission();
  }, [checked]);
  ...
  return (
    <div>
      ...
      <DialogConfirm
        ...
        message="Create new role in system"
        open={dialogConfirm}
      />
    </div>
  );
